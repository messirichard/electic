...
class LVCA_Module {
    ...
    function get_thumbnail($size = 'custom') {
        ...
        if ($thumbnail_exists = has_post_thumbnail($this->post_ID)):
            ...
            $output .= '<div class="lvca-module-thumb">';

            $output .= $this->get_media($size);

            $output .= $this->get_lightbox();

            $output .= '</div><!-- .lvca-module-thumb -->';
    ...
    function get_media($size = 'custom') {
        ...
        if ($this->settings['image_linkable']):
            ...
            $output .= '<a class="lvca-post-link" href="' . get_the_permalink($this->post_ID) . '"' . $target . '>' . get_the_post_thumbnail($this->post_ID, $image_size) . '</a>';

        else:

            $output .= get_the_post_thumbnail($this->post_ID, $image_size);
    ...
    function get_lightbox() {
        ...
        if ($this->settings['enable_lightbox']) :
            ...
            if ($featured_image_data) {
                ...
                $output .= '<a class="lvca-lightbox-item" data-fancybox="' . esc_attr($this->settings['block_class']) . '" data-post-link="' . esc_url(get_the_permalink($this->post_ID)) . '" data-post-excerpt="' . esc_html($this->get_excerpt_for_lightbox()) . '" href="' . $featured_image_src . '" title="' . get_the_title($this->post_ID) . '"><i class="lvca-icon-full-screen"></i></a>';
    ...
    function get_media_title() {
        ...
        if ($this->settings['display_title_on_thumbnail']) :
            ...
            $output = '<h3 class="lvca-post-title">';
    ...
    function get_media_overlay() {
        ...
        $output = '<div class="lvca-module-image-overlay"></div>';
    ...
    function get_read_more_link() {
        ...
        if ($this->settings['display_read_more']) {
            ...
            $output .= '<div class="lvca-read-more">';

            $output .= '<a href="' . get_the_permalink($this->post_ID) . '">' . esc_html__('Read more', 'livemesh-vc-addons') . '</a>';
    ...
    function get_author() {
        ...
        if ($this->settings['display_author']) :
            ...
            $output .= '<span class="author vcard">' . esc_html__('By ', 'livemesh-vc-addons') . '<a class="url fn n" href="' . get_author_posts_url($this->post->post_author) . '" title="' . esc_attr(get_the_author_meta('display_name', $this->post->post_author)) . '">' . esc_html(get_the_author_meta('display_name', $this->post->post_author)) . '</a></span>';
    ...
    function get_date($format = null) {
        ...
        if ($this->settings['display_post_date']) :
            ...
            $output .= '<span class="published"><abbr title="' . get_the_time(esc_html__('l, F, Y, g:i a', 'livemesh-vc-addons'), $this->post_ID) . '">' . get_the_time($format, $this->post_ID) . '</abbr></span>';
    ...
    function entry_comments_link($id, $args = array()) {
        ...
        $defaults = array('zero' => __('No Comments', 'livemesh-vc-addons'), 'one' => __('%1$s Comment', 'livemesh-vc-addons'), 'more' => __('%1$s Comments', 'livemesh-vc-addons'), 'css_class' => 'lvca-comments', 'none' => '', 'before' => '', 'after' => '');
        ...
        elseif (0 == $num_of_comments)
            $comments_link .= '<a href="' . get_permalink($id) . '#respond" title="' . sprintf(esc_attr__('Comment on %1$s', 'livemesh-vc-addons'), the_title_attribute(array('echo' => false, 'post' => $id))) . '">' . sprintf($args['zero'], number_format_i18n($num_of_comments)) . '</a>';
        elseif (1 == $num_of_comments)
            $comments_link .= '<a href="' . get_comments_link($id) . '" title="' . sprintf(esc_attr__('Comment on %1$s', 'livemesh-vc-addons'), the_title_attribute(array('echo' => false, 'post' => $id))) . '">' . sprintf($args['one'], number_format_i18n($num_of_comments)) . '</a>';
        elseif (1 < $num_of_comments)
            $comments_link .= '<a href="' . get_comments_link($id) . '" title="' . sprintf(esc_attr__('Comment on %1$s', 'livemesh-vc-addons'), the_title_attribute(array('echo' => false, 'post' => $id))) . '">' . sprintf($args['more'], number_format_i18n($num_of_comments)) . '</a>';
