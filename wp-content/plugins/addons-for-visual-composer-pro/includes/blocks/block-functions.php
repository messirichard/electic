...
add_action('wp_ajax_lvca_load_posts_block', 'lvca_load_posts_block_callback');

add_action('wp_ajax_nopriv_lvca_load_posts_block', 'lvca_load_posts_block_callback');


function lvca_load_posts_block_callback() {
    ...
    if (!empty($ajax_parameters['query']))
        $query_params = lvca_parse_block_query($ajax_parameters);


    if (!empty($_POST['settings']))
        $settings = lvca_parse_block_settings($ajax_parameters['settings']);

    $loop = new WP_Query($query_params);

    $block = LVCA_Blocks_Manager::get_instance($ajax_parameters['blockType']);
...
function lvca_parse_block_query($params) {
...
function lvca_parse_block_settings($settings) {
    ...
    $s['image_size'] = isset($s['image_size']) ? $s['image_size'] : 'large';
    ...
    // $s['show_related_posts'] = filter_var($s['show_related_posts'], FILTER_VALIDATE_BOOLEAN);

    // $s['current_post_id'] = filter_var($s['current_post_id'], FILTER_VALIDATE_INT);
